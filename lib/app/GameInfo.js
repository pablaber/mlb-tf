...
const { POSITIONS, DATE_FORMATS, ERROR_MESSAGES } = require('../config/constants');
...
module.exports = class GameInfo {
   ...
   * @return {GameInfo}
   */
  static async loadFromGameObject(client, gameObject, dateCheck, loadLineup = false) {
    const gameInfo = new GameInfo();
    const { schedule, score } = gameObject;
    if (schedule.scheduleStatus === 'POSTPONED') {
      const playedToday = moment(schedule.startTime).dayOfYear() === dateCheck;
      precond.checkState(playedToday, ERROR_MESSAGES.POSTPONED);
    }
